...
dataset_visualize <- function(
  ...
  if(!is.logical(.keep_files)) stop('`.keep_files` must be TRUE of FALSE (TRUE by default)')
...
datatable(data_dict$Categories, options = list(scrollX = TRUE),rownames = FALSE)
...
') %>% write_lines(file = paste0(to,"/temp_bookdown_report/file/bookdown-template-master/index.Rmd"), append = TRUE)
  ...
  suppressWarnings(suppressMessages(try({file.remove(list.files() %>% str_subset(dataset[1] %>% names))}, silent = TRUE)))
...
identify_visual_type <- function(dataset, data_dict){
  ...
  try({
    ...
    if(nrow(var_names_cat_dd) == 0){
      ...
      name_var <- names(dataset[names(dataset) %in% (var_names_cat_dd$variable)])
      ...
      to_eval <-
        var_name_cat_dataset %>% inner_join(var_names_cat_dd, by = "variable") %>%
        mutate(to_eval = paste0("all(",.data$`code_dataset`," %in% ",.data$`code_dd`,")"))

      to_eval <-
        to_eval %>% rowwise %>%
        mutate(to_eval = eval(parse(text = str_squish(.data$`to_eval`) %>% str_remove_all("\\\r")))) %>%
...
identify_plot_type <- function(dataset = NULL, data_dict, group_by = NULL, out = "plotly"){
  ...
  data_dict$Variables <-
    ...
    mutate(
      plot_3 = case_when(
        .data$`viz_type` == "categorical"                       ~ paste0('fabR::plot_pie(      tbl = dataset,col = "',.data$`name`,'" ,                                 out = "',out,'-code", group_by = ',group_by,')'),
        TRUE                      ~ "'message(\"\")'"))

  data_dict$Variables <-
    data_dict$Variables %>%
    mutate(plot_4 =                              paste0('fabR::fabR::plot_pie_valid_value(   tbl = dataset,col = "',.data$`name`,'" , missing_values = "',.data$`code_dd`,'", out = "',out,'-code", group_by = ',group_by,')'))
